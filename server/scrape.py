...
def download_transaction_list(driver: webdriver.Firefox, from_date: int, to_date: int) -> None:
    ...
    def _choose_date_in_calendar(driver: webdriver.Firefox, date: int) -> None:
        if not isinstance(date, int) or date <= 0 or date > 31:
            print('Invalid date, date should be a number between 1 and bound of the current month')
            return
